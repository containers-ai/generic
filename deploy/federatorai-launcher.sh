...
get_build_tag()
{
    ...
    if [ "$tag_number" = "$default" ]; then
        ...
        latest_tag=$(curl -s https://raw.githubusercontent.com/containers-ai/generic/master/deploy/manifest/version.txt|cut -d '=' -f2)
        if [ "$latest_tag" = "" ]; then
...
check_previous_build_tag_and_do_download_for_upgrade(){
    ...
    if [ "0$previous_tag_first_digit" -ge "5" ]; then
        ...
        if ! curl -sL --fail https://raw.githubusercontent.com/containers-ai/generic/${previous_fed_tag}/deploy/backup-restore.sh -O; then
